            ...
            <div class="media-left">
                <figure class="image is-64x64">
                    <template v-if="intersected">
                        <img class="is-rounded" v-if="card.profile_img == null || card.profile_img == ''" :src="profile_alt_img" alt="profile">
                        <img class="is-rounded" v-else :src="card.profile_img" :alt="profile_alt_img">
                    </template>
                    <b-skeleton circle width="64px" height="64px" :active="!intersected"></b-skeleton>
                </figure>
            </div>
            <div class="media-content" >
                    <div class="title is-4">{{card.title}}</div>
                    <div class="subtitle is-6">{{card.author}}</div>
            </div>
        </div>
    </div>
